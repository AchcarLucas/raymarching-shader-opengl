...
float getDist(vec3 p)
{
    float sdA = drawSphere(p, vec3(0.0, 1.0, 0.0), 1.0);
    ...
    float d = min(
        // diminuir o comprimento do ray marching para não ser afetado pela ondulação do plano
        drawPlane(p - vec3(0, 0, 0), vec3(0, 1, 0), sin(p.x * 2.5)) * 0.5,
        sdB
    );
